...
export function accountingRetrieveInvoiceLineItem(
  client: MaesnCore,
  request: operations.GetLineItemRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetLineItemResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.GetLineItemRequest$outboundSchema, value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetLineItemResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
