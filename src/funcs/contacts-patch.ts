...
export function contactsPatch(
  client: MaesnCore,
  request: operations.PatchContactRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.PatchContactResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.PatchContactRequest$outboundSchema, value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.PatchContactResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
