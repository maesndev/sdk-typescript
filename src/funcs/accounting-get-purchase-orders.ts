...
export function accountingGetPurchaseOrders(
  ...
  Result<
    operations.GetPurchaseOrdersResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        z.optional(operations.GetPurchaseOrdersRequest$outboundSchema),
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetPurchaseOrdersResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
