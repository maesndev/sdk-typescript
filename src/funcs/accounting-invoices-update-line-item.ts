...
export function accountingInvoicesUpdateLineItem(
  client: MaesnCore,
  request: operations.PatchLineItemRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.PatchLineItemResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.PatchLineItemRequest$outboundSchema, value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.PatchLineItemResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
