...
export function accountingFetchJournals(
  client: MaesnCore,
  request?: operations.GetJournalsRequest | undefined,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetJournalsResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(z.optional(operations.GetJournalsRequest$outboundSchema), value),
  ...
  const path = pathToFunc("/accounting/journals")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetJournalsResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
