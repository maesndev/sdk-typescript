...
export function accountingRetrieveOfferLineItem(
  client: MaesnCore,
  request: operations.GetOfferLineItemRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetOfferLineItemResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.GetOfferLineItemRequest$outboundSchema, value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetOfferLineItemResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
