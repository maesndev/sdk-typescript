...
export function accountingAmendInvoice(
  client: MaesnCore,
  request: operations.PatchInvoiceRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.PatchInvoiceResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.PatchInvoiceRequest$outboundSchema, value),
  ...
  const path = pathToFunc("/accounting/invoices/{invoiceId}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.PatchInvoiceResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
