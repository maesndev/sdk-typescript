...
export function accountingCreateSupplier(
  client: MaesnCore,
  request: operations.CreateSupplierRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.CreateSupplierResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.CreateSupplierRequest$outboundSchema, value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, operations.CreateSupplierResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
