...
export function tenantsDeleteEndUser(
  client: MaesnCore,
  request?: operations.DeleteEndUserRequest | undefined,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.DeleteEndUserResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        z.optional(operations.DeleteEndUserRequest$outboundSchema),
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.DeleteEndUserResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
