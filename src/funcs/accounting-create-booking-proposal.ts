...
export function accountingCreateBookingProposal(
  client: MaesnCore,
  request: operations.CreateBookingProposalRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.CreateBookingProposalResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.CreateBookingProposalRequest$outboundSchema, value),
  ...
  const path = pathToFunc("/accounting/bookingProposals")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, operations.CreateBookingProposalResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
