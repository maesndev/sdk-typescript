...
export function accountingRetrieveItems(
  client: MaesnCore,
  request?: operations.GetItemsRequest | undefined,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetItemsResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(z.optional(operations.GetItemsRequest$outboundSchema), value),
  ...
  const path = pathToFunc("/accounting/items")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetItemsResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
