...
export function accountingFetchBill(
  client: MaesnCore,
  request: operations.GetBillRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetBillResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.GetBillRequest$outboundSchema, value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetBillResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
