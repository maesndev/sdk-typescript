...
export function accountingFetchExpenses(
  client: MaesnCore,
  request?: operations.GetExpensesRequest | undefined,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetExpensesResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(z.optional(operations.GetExpensesRequest$outboundSchema), value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetExpensesResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
