...
export function accountingCreateExpenseAsync(
  client: MaesnCore,
  request: operations.CreateExpenseAsyncRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.CreateExpenseAsyncResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.CreateExpenseAsyncRequest$outboundSchema, value),
  ...
  const path = pathToFunc("/accounting/expenses/async")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(202, operations.CreateExpenseAsyncResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
