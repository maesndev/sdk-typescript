...
export function accountingJournalEntriesGetAttachments(
  client: MaesnCore,
  request: operations.GetJournalEntryAttachmentsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetJournalEntryAttachmentsResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        operations.GetJournalEntryAttachmentsRequest$outboundSchema,
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetJournalEntryAttachmentsResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
