...
export function accountingFetchOffers(
  client: MaesnCore,
  request?: operations.GetOffersRequest | undefined,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetOffersResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(z.optional(operations.GetOffersRequest$outboundSchema), value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetOffersResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
