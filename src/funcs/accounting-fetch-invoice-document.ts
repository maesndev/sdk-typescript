...
export function accountingFetchInvoiceDocument(
  client: MaesnCore,
  request: operations.GetInvoiceDocumentRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetInvoiceDocumentResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.GetInvoiceDocumentRequest$outboundSchema, value),
  ...
  const path = pathToFunc("/accounting/invoices/{invoiceId}/document")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetInvoiceDocumentResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
