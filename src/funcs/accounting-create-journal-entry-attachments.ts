...
export function accountingCreateJournalEntryAttachments(
  client: MaesnCore,
  request: operations.CreateJournalEntryAttachmentsRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.CreateJournalEntryAttachmentsResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        operations.CreateJournalEntryAttachmentsRequest$outboundSchema,
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, operations.CreateJournalEntryAttachmentsResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
