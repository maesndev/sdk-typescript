...
export function accountingGetFiscalYears(
  client: MaesnCore,
  request?: operations.GetFiscalYearsRequest | undefined,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetFiscalYearsResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        z.optional(operations.GetFiscalYearsRequest$outboundSchema),
        value,
      ),
  ...
  const path = pathToFunc("/accounting/fiscalYears")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetFiscalYearsResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
