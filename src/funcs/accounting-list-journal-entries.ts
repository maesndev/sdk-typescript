...
export function accountingListJournalEntries(
  client: MaesnCore,
  request?: operations.GetJournalEntriesRequest | undefined,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetJournalEntriesResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        z.optional(operations.GetJournalEntriesRequest$outboundSchema),
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetJournalEntriesResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
