...
export function accountingFetchItem(
  client: MaesnCore,
  request: operations.GetItemRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetItemResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.GetItemRequest$outboundSchema, value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetItemResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
