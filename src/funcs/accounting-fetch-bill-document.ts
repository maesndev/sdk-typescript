...
export function accountingFetchBillDocument(
  client: MaesnCore,
  request: operations.GetBillDocumentRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetBillDocumentResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.GetBillDocumentRequest$outboundSchema, value),
  ...
  const path = pathToFunc("/accounting/bills/{billId}/document")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetBillDocumentResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
