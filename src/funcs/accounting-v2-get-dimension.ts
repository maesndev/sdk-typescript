...
export function accountingV2GetDimension(
  client: MaesnCore,
  request: operations.GetDimensionsByDimensionRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetDimensionsByDimensionResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.GetDimensionsByDimensionRequest$outboundSchema, value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetDimensionsByDimensionResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
