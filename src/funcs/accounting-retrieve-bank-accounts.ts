...
export function accountingRetrieveBankAccounts(
  client: MaesnCore,
  request?: operations.GetBankAccountsRequest | undefined,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetBankAccountsResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        z.optional(operations.GetBankAccountsRequest$outboundSchema),
        value,
      ),
  ...
  const path = pathToFunc("/accounting/bankAccounts")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetBankAccountsResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
