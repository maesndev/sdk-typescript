...
export function accountingPatchJournalEntry(
  client: MaesnCore,
  request: operations.PatchJournalEntryRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.PatchJournalEntryResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.PatchJournalEntryRequest$outboundSchema, value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.PatchJournalEntryResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
