...
export function accountingDeleteEventSubscriptions(
  client: MaesnCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.DeleteEventSubscriptionsRequest$outboundSchema, value),
  ...
  const pathParams = {
    eventSubscriptionId: encodeSimple(
      "eventSubscriptionId",
      payload.eventSubscriptionId,
      { explode: false, charEncoding: "percent" },
    ),
  };
  const path = pathToFunc(
    "/accounting/event-subscriptions/{eventSubscriptionId}",
  )(pathParams);
