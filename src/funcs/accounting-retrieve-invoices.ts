...
export function accountingRetrieveInvoices(
  client: MaesnCore,
  request?: operations.GetInvoicesRequest | undefined,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetInvoicesResponse,
    | MaesnError
...
> {
  ...
  const query = encodeFormQuery({
    ...
    "dueDateFrom": payload?.dueDateFrom,
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetInvoicesResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
