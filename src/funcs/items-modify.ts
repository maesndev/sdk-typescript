...
export function itemsModify(
  client: MaesnCore,
  request: operations.PatchItemRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.PatchItemResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    request,
    (value) => z.parse(operations.PatchItemRequest$outboundSchema, value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.PatchItemResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
