...
export function tenantsGenerateTenantSigningSecret(
  client: MaesnCore,
  request?: operations.GenerateTenantSigningSecretRequest | undefined,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GenerateTenantSigningSecretResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        z.optional(
          operations.GenerateTenantSigningSecretRequest$outboundSchema,
        ),
        value,
      ),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, operations.GenerateTenantSigningSecretResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
