...
export function authGetCompanies(
  ...
  Result<
    operations.GetCompaniesResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(z.optional(operations.GetCompaniesRequest$outboundSchema), value),
  ...
  const path = pathToFunc("/auth/companies")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetCompaniesResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
