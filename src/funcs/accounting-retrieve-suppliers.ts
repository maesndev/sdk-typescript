...
export function accountingRetrieveSuppliers(
  client: MaesnCore,
  request?: operations.GetSuppliersRequest | undefined,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetSuppliersResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(z.optional(operations.GetSuppliersRequest$outboundSchema), value),
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetSuppliersResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
