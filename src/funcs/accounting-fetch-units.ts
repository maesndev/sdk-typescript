...
export function accountingFetchUnits(
  client: MaesnCore,
  request?: operations.GetUnitsRequest | undefined,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetUnitsResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(z.optional(operations.GetUnitsRequest$outboundSchema), value),
  ...
  const path = pathToFunc("/accounting/units")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetUnitsResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
