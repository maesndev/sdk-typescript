...
export function accountingCreateJournalEntry(
  client: MaesnCore,
  request: operations.CreateJournalEntryRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.CreateJournalEntryResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      z.parse(operations.CreateJournalEntryRequest$outboundSchema, value),
  ...
  if (payload.body.filesPos !== undefined) {
    appendForm(body, "filesPos", payload.body.filesPos);
  }
  ...
  const path = pathToFunc("/accounting/journalEntries")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, operations.CreateJournalEntryResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
