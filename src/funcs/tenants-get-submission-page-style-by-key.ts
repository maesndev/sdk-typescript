...
export function tenantsGetSubmissionPageStyleByKey(
  client: MaesnCore,
  request?: operations.GetSubmissionPageStyleByKeyRequest | undefined,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.GetSubmissionPageStyleByKeyResponse,
    | MaesnError
...
> {
  const parsed = safeParse(
    ...
    (value) =>
      z.parse(
        z.optional(
          operations.GetSubmissionPageStyleByKeyRequest$outboundSchema,
        ),
        value,
      ),
  ...
  const path = pathToFunc("/tenants/submission-page-style-by-key")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.GetSubmissionPageStyleByKeyResponse$inboundSchema),
    M.fail("4XX"),
    M.fail("5XX"),
  )(response, req);
  if (!result.ok) {
