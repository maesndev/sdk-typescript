...
import { MaesnDefaultError } from "../models/errors/maesn-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | MaesnDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | MaesnDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new MaesnDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new MaesnDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
