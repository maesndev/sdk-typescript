...
async function run() {
  const result = await maesn.files.getDocumentExtensions();
...
import { filesGetDocumentExtensions } from "@maesn/typescript-sdk/funcs/files-get-document-extensions.js";
...
async function run() {
  const res = await filesGetDocumentExtensions(maesn);
  if (res.ok) {
    const { value: result } = res;
    console.log(result);
  } else {
    console.log("filesGetDocumentExtensions failed:", res.error);
  }
}
