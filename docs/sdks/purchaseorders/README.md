...
async function run() {
  const result = await maesn.purchaseOrders.list();
...
import { purchaseOrdersList } from "@maesn/typescript-sdk/funcs/purchase-orders-list.js";
...
async function run() {
  const res = await purchaseOrdersList(maesn);
  if (res.ok) {
