...
async function run() {
  const result = await maesn.accounting.journalEntries.getAttachments({
    journalEntryId: "<id>",
  });
...
import { accountingJournalEntriesGetAttachments } from "@maesn/typescript-sdk/funcs/accounting-journal-entries-get-attachments.js";
...
async function run() {
  const res = await accountingJournalEntriesGetAttachments(maesn, {
    journalEntryId: "<id>",
  });
  if (res.ok) {
    ...
    console.log("accountingJournalEntriesGetAttachments failed:", res.error);
  }
}
